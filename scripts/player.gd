...
var maximun_rotation_up := -30.0
var maximun_rotation_down := 90.0
...
func _physics_process(_delta: float) -> void:
	...
	if rotation_degrees <= maximun_rotation_up:
		rotation_degrees = maximun_rotation_up
		angular_velocity = 0.0
	
	if linear_velocity.y > 0.0:
		if rotation_degrees <= maximun_rotation_down:
